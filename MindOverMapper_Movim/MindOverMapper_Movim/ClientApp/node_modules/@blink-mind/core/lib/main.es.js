...
var Topic = /** @class */ (function (_super) {
    ...
    Topic.create = function (_a) {
        var key = _a.key, _b = _a.parentKey, parentKey = _b === void 0 ? null : _b, _c = _a.content, content = _c === void 0 ? 'new node' : _c, _d = _a.subKeys, subKeys = _d === void 0 ? [] : _d, _e = _a.collapse, collapse = _e === void 0 ? false : _e;
...
function setContent(_a) {
    var model = _a.model, topicKey = _a.topicKey, content = _a.content;
    var topic = model.getTopic(topicKey);
    if (topic) {
        var _b = topic.getBlock(BlockType.CONTENT), index = _b.index, block = _b.block;
        if (block.data !== content) {
            model = model.updateIn(['topics', topicKey, 'blocks', index, 'data'], function (data) { return content; });
        }
        return focusTopic({
            model: model,
            topicKey: topicKey,
...
function setDesc(_a) {
    var model = _a.model, topicKey = _a.topicKey, desc = _a.desc;
    var topic = model.getTopic(topicKey);
    if (topic) {
        ...
        if (index === -1) {
            ...
            if (block.data !== desc) {
                model = model.updateIn(['topics', topicKey, 'blocks', index, 'data'], function (data) { return desc; });
            }
        }
        return focusTopic({
            model: model,
...
export { Block, BlockType, Config, Controller, Data, DiagramLayoutType, FocusMode, Model, index as ModelModifier, index as Modifiers, Topic, TopicDirection, TopicRelationship, TopicVisualLevel, defaultTheme, getAllSubTopicKeys, getRelationship, index$1 as models };
