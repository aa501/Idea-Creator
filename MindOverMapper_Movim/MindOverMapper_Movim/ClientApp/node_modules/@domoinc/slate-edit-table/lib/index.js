...
function createRow(opts, columns, getCellContent) {
  ...
  return slate.Block.create({
    type: opts.typeRow,
    nodes: slate.Block.createList(cellNodes)
  });
}
...
function createTable(opts, columns, rows, getCellContent) {
  var rowNodes = immutable.Range(0, rows).map(function (i) {
    return createRow(opts, columns, getCellContent ? getCellContent.bind(null, i) : undefined);
  }).toJS();
  return slate.Block.create({
    type: opts.typeTable,
    nodes: slate.Block.createList(rowNodes)
  });
}
...
function clearCell(opts, change, cell) {
  var newBlock = slate.Block.create({
    type: opts.typeContent
  });
...
function removeTableByKey(opts, editor, key) {
  ...
  if (nextBlock) {
    ...
    if (prevBlock) {
      ...
      nextFocusBlock = slate.Block.create({
...
function onCopy(opts, event, editor, next) {
  ...
  var tempEditor = new slate.Editor({
    value: slate.Value.fromJSON({
      document: fragment
    })
  });
  tempEditor.moveToRangeOfDocument();
  slateReact.cloneFragment(event, tempEditor);
}
